...
def plot_collectors_curve(args, start_times, read_lengths):
    ...
    start_times = [
        float(t - t_0) / float(3600) + 0.00000001 for t in start_times
    ]
    ...
    d = {
        'start': [start_times[n] for n in range(0, len(start_times), step)],
        'lengths':
        [read_lengths[n] for n in range(0, len(read_lengths), step)],
